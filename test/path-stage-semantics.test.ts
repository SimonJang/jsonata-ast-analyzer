...
describe("path-stage semantics", () => {
  ...
  it("preserves bare reads in direct focus-bound mixed alias projections", () => {
    expect(
      sortPaths(
        extractPaths(
          '(flag ? {"x": primary} : fallback)@$row.($row.x.enabled ? $row.x.name : missing)',
        ),
      ),
    ).toEqual(
      sortPaths([
  ...
  it("uses direct object alias suffixes as function context", () => {
    expect(
      ...
      sortPaths([
        { path: "orders.items.price", confidence: "static" },
        { path: "orders.items.price.amount", confidence: "static" },
      ]),
    );
  });
  ...
  it("uses direct array result alias suffixes as projection context", () => {
    expect(
      sortPaths(extractPaths('([primary, fallback]).x.{"out": name}.out')),
    ).toEqual(
      sortPaths([
        { path: "fallback", confidence: "static" },
        { path: "fallback.x.name", confidence: "static" },
  ...
  it("keeps focus bindings on object constructor predicates", () => {
  ...
  it("keeps focus bindings on array constructor group entries", () => {
    expect(sortPaths(extractPaths("[items]@$v{$v.category: $v.total}"))).toEqual(
      sortPaths([
        { path: "items", confidence: "static" },
  ...
  it("keeps focus bindings on object constructor group entries", () => {
    expect(
      sortPaths(extractPaths('{"x": items}@$v{$v.x.category: $v.x.total}')),
    ).toEqual(
      sortPaths([
        { path: "items", confidence: "static" },
        { path: "items.category", confidence: "static" },
  ...
  it("keeps focus bindings on sorted array constructor group entries", () => {
    expect(
      sortPaths(extractPaths("[items]@$v^(<$v.rank){$v.category: $v.total}")),
    ).toEqual(
      sortPaths([
        { path: "items", confidence: "static" },
        { path: "items.category", confidence: "static" },
  ...
  it("keeps indexed variable stage predicate reads", () => {
    expect(sortPaths(extractPaths("($items := orders.items; $items#$i[active])"))).toEqual(
      sortPaths([
        { path: "orders.items", confidence: "static" },
        { path: "orders.items.active", confidence: "static" },
      ]),
  ...
  it("keeps focus-bound nested block predicate reads contextual", () => {
    expect(sortPaths(extractPaths("orders.items.(price)@$v[$v.active]"))).toEqual(
      sortPaths([
        { path: "orders.items.price", confidence: "static" },
        { path: "orders.items.price.active", confidence: "static" },
      ]),
  ...
  it("resolves indexed nested block group object aliases contextually", () => {
    expect(
      sortPaths(extractPaths('orders.items.({"x": price})#$i{x.category: x.total}')),
    ).toEqual(
      sortPaths([
        { path: "orders.items.price", confidence: "static" },
        { path: "orders.items.price.category", confidence: "static" },
  ...
  it("resolves sorted nested block object aliases contextually", () => {
    expect(
      sortPaths(
        extractPaths(
          'orders.items.({"x": price})#$i[x.active]^(<x.rank){x.category: x.total}',
  ...
  it("resolves sorted variable-bound nested block object group aliases contextually", () => {
    expect(
      sortPaths(
        extractPaths(
          '($p := orders.items.({"x": price}); $p#$i[x.active]^(<x.rank){x.category: x.total})',
        ),
      ),
    ).toEqual(
      sortPaths([
